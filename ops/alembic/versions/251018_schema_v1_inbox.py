...
def upgrade() -> None:
    ...
    op.create_table('inbox_items',
        ...
        sa.CheckConstraint("status IN ('received','validated','parsed','error')", name='ck_inbox_items__status_valid'),
        schema='zero_admin'
    )

    # parsed_items table - parsed content from inbox items
    op.create_table('parsed_items',
        sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('tenant_id', sa.UUID(), nullable=False),
        ...
        sa.ForeignKeyConstraint(['inbox_item_id'], ['zero_admin.inbox_items.id'], name='fk_inbox_item_id_inbox_items', ondelete='CASCADE'),
        schema='zero_admin'
    )
    ...
    op.create_table('chunks',
        ...
        sa.ForeignKeyConstraint(['parsed_item_id'], ['zero_admin.parsed_items.id'], name='fk_parsed_item_id_parsed_items', ondelete='CASCADE'),
    ...
    op.create_table('event_outbox',
        ...
        sa.CheckConstraint("status IN ('pending','processing','sent','failed','dlq')", name='ck_event_outbox__status_valid'),
        sa.UniqueConstraint('tenant_id', 'idempotency_key', 'event_type', name='uq_event_outbox__idem'),
        schema='zero_admin'
    )
    ...
    op.create_table('processed_events',
        ...
        sa.Column('event_type', sa.String(length=128), nullable=False),
        sa.Column('idempotency_key', sa.String(length=128), nullable=False),
        sa.Column('processed_at', sa.TIMESTAMP(timezone=True), server_default=sa.text("timezone('utc', now())"), nullable=False),
        sa.PrimaryKeyConstraint('tenant_id', 'event_type', 'idempotency_key', name='pk_processed_events'),
        schema='zero_admin'
    )
    ...
    op.create_table('dead_letters',
        ...
        sa.Column('failed_at', sa.TIMESTAMP(timezone=True), server_default=sa.text("timezone('utc', now())"), nullable=False),
        sa.PrimaryKeyConstraint('id', name='pk_dead_letters'),
        schema='zero_admin'
    )

    # Additional unique constraint for inbox_items content_hash
    op.create_unique_constraint('uq_inbox_items__content_hash', 'inbox_items', ['tenant_id', 'content_hash'], schema='zero_admin')

    # Create indexes for query optimization
    op.create_index('ix_inbox_items_created_at', 'inbox_items', ['tenant_id', 'created_at'], schema='zero_admin')
    op.create_index('ix_inbox_items_status', 'inbox_items', ['tenant_id', 'status'], schema='zero_admin')
    op.create_index('ix_parsed_items_created_at', 'parsed_items', ['tenant_id', 'created_at'], schema='zero_admin')
    op.create_index('ix_parsed_items_inbox_item_id', 'parsed_items', ['tenant_id', 'inbox_item_id'], schema='zero_admin')
    op.create_index('ix_chunks_parsed_item_id', 'chunks', ['tenant_id', 'parsed_item_id'], schema='zero_admin')
    op.create_index('ix_chunks_seq_no', 'chunks', ['tenant_id', 'seq_no'], schema='zero_admin')
    op.create_index('ix_event_outbox_created_at', 'event_outbox', ['tenant_id', 'created_at'], schema='zero_admin')
    op.create_index('ix_event_outbox_status', 'event_outbox', ['tenant_id', 'status'], schema='zero_admin')
