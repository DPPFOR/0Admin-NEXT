...
def upgrade() -> None:
    ...
    if not result.scalar():
        ...
        op.create_index(
            "ix_parsed_items_tenant_id", "parsed_items", ["tenant_id"], schema="inbox_parsed"
        )
        op.create_index(
            "ix_parsed_items_created_at", "parsed_items", ["created_at"], schema="inbox_parsed"
        )
        op.create_index(
            "ix_parsed_items_doctype", "parsed_items", ["doctype"], schema="inbox_parsed"
        )
        op.create_index(
            "ix_parsed_items_quality_status",
            "parsed_items",
            ["quality_status"],
            schema="inbox_parsed",
        )
    ...
    if not result.scalar():
        ...
        op.create_index(
            "ix_parsed_item_chunks_parsed_item_id",
            "parsed_item_chunks",
            ["parsed_item_id"],
            schema="inbox_parsed",
        )
...
def downgrade() -> None:
    ...
    # Drop triggers
    op.execute(
        "DROP TRIGGER IF EXISTS update_parsed_item_chunks_updated_at ON inbox_parsed.parsed_item_chunks"
    )
