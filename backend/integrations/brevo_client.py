...
class BrevoClient:
    ...
    def send_transactional(
        ...
        if to and self.is_hard_bounced(to):
            self.logger.warning(
                f"Skipping email to hard-bounced address",
                extra={"tenant_id": tenant_id, "to": to}
            )
            return BrevoResponse(
                success=False,
                error="Email address is on hard-bounce list",
                dry_run=False
            )
        ...
        if to and not dry_run:
            ...
            if not can_retry:
                self.logger.warning(
                    f"Soft-bounce policy exceeded - moving to hard-bounce",
                    extra={"tenant_id": tenant_id, "to": to, "reason": reason}
                )
                self.add_hard_bounce(to)
                return BrevoResponse(
                    success=False,
                    error=f"Soft-bounce policy exceeded: {reason}",
                    dry_run=False
                )
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if response.status_code == 400 and "invalid" in response.text.lower():
                    self.logger.error(
                        f"Hard bounce detected - adding to blocklist",
    ...
    def _handle_dry_run(self, to: str, subject: str, tenant_id: str) -> BrevoResponse:
        ...
        """
        self.logger.info(
            f"DRY-RUN: Would send email via Brevo",
            extra={
                "tenant_id": tenant_id,
                "to": to,
                "subject": subject[:50] + "..." if len(subject) > 50 else subject,
        ...
        return BrevoResponse(
            success=True,
            message_id=f"dry-run-{tenant_id}-{hash(to)}",
            dry_run=True
        )
    ...
    def _check_soft_bounce_policy(self, email: str) -> tuple[bool, Optional[str]]:
        """Check soft-bounce policy: max 3 attempts in 72h.
        ...
        if len(attempts_in_window) >= 3:
            return False, f"Max 3 soft-bounce attempts in 72h exceeded ({len(attempts_in_window)} attempts)"
    ...
    def record_soft_bounce(self, email: str) -> None:
        ...
        if not can_retry:
            self.logger.warning(
                f"Soft-bounce policy exceeded - promoting to hard-bounce",
            ...
            self.logger.info(
                f"Soft-bounce recorded",
                extra={"email": email, "attempt": attempt_count, "max": 3}
            )
