"""Tests for Brevo webhook HMAC verification."""
...
def test_verify_hmac_valid(secret, payload):
...
def test_verify_hmac_invalid_signature(secret, payload):
...
def test_verify_hmac_no_secret(payload):
    """Test HMAC verification without secret."""
    body = json.dumps(payload).encode("utf-8")
...
def test_verify_hmac_no_signature_header(secret, payload):
    """Test HMAC verification without signature header."""
    body = json.dumps(payload).encode("utf-8")

    assert verify_hmac_signature(body, None, secret) is False


def test_verify_hmac_wrong_format(secret, payload):
    """Test HMAC verification with wrong header format."""
    body = json.dumps(payload).encode("utf-8")
...
def test_webhook_valid_hmac(client, secret, payload, monkeypatch):
    ...
    response = client.post(
        ...
        content=body,
        headers={"X-Brevo-Signature": signature_header, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001"},
    )

    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "ok"
    # Event ID may be None if duplicate, but status should be ok
    assert data["tenant_id"] == "00000000-0000-0000-0000-000000000001"


def test_webhook_invalid_hmac(client, secret, payload, monkeypatch):
...
def test_webhook_no_signature(client, payload, monkeypatch):
    """Test webhook endpoint without signature header."""
    monkeypatch.setenv("BREVO_WEBHOOK_SECRET", "test-secret")
...
def test_webhook_invalid_json(client, secret, monkeypatch):
