...
from agents.mahnwesen.dto import DunningEvent, DunningNotice, DunningStage, DunningChannel
...
class TestEventDispatch:
    ...
    def test_publish_dunning_issued_failure(self, outbox_client, sample_event):
        ...
        with patch.object(outbox_client, '_simulate_outbox_write', side_effect=Exception("DB Error")):
            result = outbox_client.publish_dunning_issued(sample_event, "CORR-001")
    ...
    @pytest.mark.parametrize("stage,expected_stage_value", [
    ...
    @pytest.mark.parametrize("channel,expected_channel_value", [
