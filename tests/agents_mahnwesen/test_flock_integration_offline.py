...
from agents.mahnwesen.playbooks import DunningContext, DunningPlaybook, TemplateEngine
...
class TestFlockIntegration:
    ...
    def test_compose_dunning_notices_task(self, playbook, context, sample_invoices):
        ...
        for notice in result["notices"]:
            ...
            assert notice.stage in [DunningStage.STAGE_1, DunningStage.STAGE_2, DunningStage.STAGE_3]
    ...
    def test_run_once_failure(self, playbook, context):
        ...
        # Mock API failure
        with patch.object(context.read_client, 'get_overdue_invoices', side_effect=Exception("API Error")):
            result = playbook.run_once(context)
    ...
    def test_error_handling(self, playbook, context):
        ...
        # Mock API error
        with patch.object(context.read_client, 'get_overdue_invoices', side_effect=Exception("Network error")):
            result = playbook.run_once(context)
