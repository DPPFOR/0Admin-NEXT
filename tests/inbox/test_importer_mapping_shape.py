...
def _load_mapper():
    spec = _iu.spec_from_file_location(
        "mapper", "backend/apps/inbox/importer/mapper.py"
    )
...
def _load_validators():
    spec = _iu.spec_from_file_location(
        "validators", "backend/apps/inbox/importer/validators.py"
    )
    mod = _iu.module_from_spec(spec)  # type: ignore[arg-type]
    assert spec and spec.loader
    spec.loader.exec_module(mod)  # type: ignore[union-attr]
...
def test_mapping_from_sample_artifact():
    data = json.loads(open("artifacts/inbox_local/samples/sample_result.json", "r", encoding="utf-8").read())
    mapper = _load_mapper()
