...
class TestViewsContract:
    ...
    def test_v_inbox_by_tenant_structure(self, engine):
        ...
        with engine.begin() as conn:
            ...
            for col_name, (expected_type, expected_nullable) in expected_columns.items():
                ...
                assert actual_nullable == expected_nullable, f"Column {col_name} nullable is {actual_nullable}, expected {expected_nullable}"
    ...
    def test_v_invoices_latest_structure(self, engine):
        ...
        with engine.begin() as conn:
            ...
            for col_name, (expected_type, expected_nullable) in expected_columns.items():
                ...
                assert actual_type in [expected_type, expected_type.replace('character varying', 'text')], \
    ...
    def test_views_data_consistency(self, engine):
        ...
        with engine.begin() as conn:
            ...
            base_count = conn.execute(text("SELECT COUNT(*) FROM inbox_parsed.parsed_items")).scalar()
            invoice_count = conn.execute(text("SELECT COUNT(*) FROM inbox_parsed.parsed_items WHERE doctype = 'invoice'")).scalar()
            
            # Get counts from views
            view_count = conn.execute(text("SELECT SUM(total_items) FROM inbox_parsed.v_inbox_by_tenant")).scalar()
            view_invoice_count = conn.execute(text("SELECT COUNT(*) FROM inbox_parsed.v_invoices_latest")).scalar()
            
            # Should be consistent
            assert view_count == base_count, f"View total_items {view_count} != base count {base_count}"
            assert view_invoice_count == invoice_count, f"View invoice count {view_invoice_count} != base invoice count {invoice_count}"
