...
class TestAuditLog:
    ...
    def test_audit_log_created_on_import(self, engine, test_tenant_id, test_artifact_path):
        ...
        with engine.begin() as conn:
            conn.execute(text("DELETE FROM ops.audit_log WHERE tenant_id = :tenant_id"), 
        ...
        with engine.begin() as conn:
            audit_count = conn.execute(
                text("SELECT COUNT(*) FROM ops.audit_log WHERE tenant_id = :tenant_id AND op = 'IMPORT_UPSERT'"),
                {"tenant_id": test_tenant_id}
            ).scalar()
    ...
    def test_audit_log_idempotent_import(self, engine, test_tenant_id, test_artifact_path):
        ...
        with engine.begin() as conn:
            ...
            assert len(audit_entries) == 2, f"Expected 2 audit log entries, got {len(audit_entries)}"
