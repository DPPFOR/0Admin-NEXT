...
WINDOW_RE = r"^(P(?=.+)(?:\d+W|\d+D(?:T(?:\d+H)?(?:\d+M)?(?:\d+S)?)?|T(?:\d+H)?(?:\d+M)?(?:\d+S)?))$"
...
def validate_schema_doc(path: Path, data: dict[str, Any], version: str, tool_id: str, problems: list[str]) -> None:
    # 1) Draft 2020-12
    if data.get("$schema") != SCHEMA_URL:
        problems.append(rule("GOV_ID_CONSISTENCY", path, "/$schema", SCHEMA_URL, str(data.get("$schema"))))
    ...
    try:
        ...
        problems.append(rule("GOV_ID_CONSISTENCY", path, "/", "valid Draft 2020-12", f"invalid: {e}"))
    ...
    def _walk_refs(node: Any, ptr: str = ""):
        if isinstance(node, dict):
            if "$ref" in node:
                refv = node["$ref"]
                if refv not in ALLOWED_REFS:
                    problems.append(rule("GOV_DEFS_USAGE", path, f"{ptr}/$ref", " one of #/$defs/*", str(refv)))
...
def main() -> int:
    ...
    for tool, version, input_p, output_p, errors_p in walk_contracts(base):
        ...
        try:
            ...
            if not enum_vals:
                problems.append(rule("GOV_ID_CONSISTENCY", errors_p, "/properties/code/enum", "non-empty", "empty"))
            if not enum_vals.issubset(ALLOWED_ERRORS):
                problems.append(rule("GOV_ID_CONSISTENCY", errors_p, "/properties/code/enum", f"subset of {sorted(ALLOWED_ERRORS)}", str(sorted(enum_vals))))
        except Exception as e:
            problems.append(err(errors_p, f"malformed errors.code enum: {e}"))
        ...
        for pth, doc in ((input_p, input_s), (output_p, output_s)):
            ...
            for k, v in props.items():
                if k == "tenant_id" and isinstance(v, dict):
                    # enforce $ref-based rule
                    if "$ref" not in v or v["$ref"] != "#/$defs/uuid":
                        problems.append(rule("GOV_DEFS_USAGE", pth, "/properties/tenant_id", "{\"$ref\": \"#/$defs/uuid\"}", json.dumps(v)))
                if k in {"cursor", "next_cursor"} and isinstance(v, dict):
                    if "$ref" not in v or v["$ref"] != "#/$defs/base64":
                        problems.append(rule("GOV_DEFS_USAGE", pth, f"/properties/{k}", "{\"$ref\": \"#/$defs/base64\"}", json.dumps(v)))
                if k == "window" and isinstance(v, dict):
                    if "$ref" not in v or v["$ref"] != "#/$defs/iso_duration":
                        problems.append(rule("GOV_DEFS_USAGE", pth, "/properties/window", "{\"$ref\": \"#/$defs/iso_duration\"}", json.dumps(v)))
                if k == "limit" and isinstance(v, dict):
                    if v.get("minimum") != 1 or v.get("maximum") != 1000:
                        problems.append(rule("GOV_ID_CONSISTENCY", pth, "/properties/limit", "min=1,max=1000", json.dumps(v)))
                if k == "ts" and isinstance(v, dict):
        ...
        if tool == "qa.run_smoke":
            ...
            if enum_vals != expected:
                problems.append(rule("GOV_ID_CONSISTENCY", input_p, "/properties/selection/enum", json.dumps(expected), json.dumps(enum_vals)))
        ...
        if not changelog_p.exists():
            ...
            try:
                ...
                if first != f"{version} – initial":
                    problems.append(rule("CHANGELOG_DATE_FORMAT", changelog_p, "/0", f"{version} – initial", first))
                second = (lines[1] if len(lines) > 1 else "").strip()
                if not re.match(r"^\d{4}-\d{2}-\d{2}$", second):
                    problems.append(rule("CHANGELOG_DATE_FORMAT", changelog_p, "/1", "YYYY-MM-DD", second))
            except Exception as e:
                problems.append(rule("CHANGELOG_DATE_FORMAT", changelog_p, "/", "readable", f"error: {e}"))
        ...
        for p in (input_p, output_p, errors_p):
            try:
                ...
                problems.append(rule("GOV_ID_CONSISTENCY", p, "/", "valid Draft 2020-12", f"invalid: {e}"))
    ...
    if readme.exists():
        ...
        def _eligible(p: str) -> bool:
            return p.startswith("docs/mcp/") or p.startswith("ops/mcp/") or p.startswith("ops/runbooks/")
