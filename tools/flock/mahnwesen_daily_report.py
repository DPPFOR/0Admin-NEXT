...
def generate_daily_report(
    ...
    try:
        ...
        if report_data["summary"]["total_overdue"] > 0:
            report_data["summary"]["avg_amount"] = report_data["summary"]["total_amount_cents"] / (report_data["summary"]["total_overdue"] * 100)
...
def save_csv_report(report_data: Dict, output_path: str) -> None:
    ...
    with open(path, 'w', encoding='utf-8', newline='') as f:
        ...
        # Header
        writer.writerow([
            "tenant_id", "report_date", "status",
            "total_overdue", "stage_1_count", "stage_2_count", "stage_3_count",
            "total_amount_cents", "unique_customers", "avg_amount"
        ])
  ...
  python tools/flock/mahnwesen_daily_report.py --tenant 00000000-0000-0000-0000-000000000001 --out /tmp/report.json
    ...
    try:
        ...
        if not args.quiet:
            ...
            if not args.out:
                print(f"\nReport saved to:")
                print(f"  JSON: artifacts/reports/mahnwesen/{args.tenant}/daily_{report_data['report_date'].replace('-', '')}.json")
                print(f"  CSV:  artifacts/reports/mahnwesen/{args.tenant}/daily_{report_data['report_date'].replace('-', '')}.csv")
