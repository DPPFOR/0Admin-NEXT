...
def load_dry_run_report(tenant_id: str, date_override: Optional[str] = None) -> Optional[Dict]:
    ...
    report_path = Path("artifacts/reports/mahnwesen") / tenant_id / f"dry_run_{report_date.strftime('%Y%m%d')}.json"
...
def print_dashboard(tenant_id: str, base_url: str = "http://localhost:8000", date_override: Optional[str] = None):
    ...
    print(f"Date: {date_override or datetime.now(timezone.utc).strftime('%Y-%m-%d')}")
    ...
    try:
        ...
        if overdue_invoices:
            ...
            stage_1 = [inv for inv in overdue_invoices if inv.get('days_overdue', 0) >= 3 and inv.get('days_overdue', 0) < 14]
            stage_2 = [inv for inv in overdue_invoices if inv.get('days_overdue', 0) >= 14 and inv.get('days_overdue', 0) < 30]
