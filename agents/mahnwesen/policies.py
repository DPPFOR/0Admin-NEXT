...
from .dto import DunningStage, DunningChannel
...
class DunningPolicies:
    ...
    def should_issue_dunning(
        ...
        if invoice.amount_cents < self.config.min_amount_cents:
            ...
            return False, f"Rechnungsbetrag {invoice.amount_decimal:.2f} EUR unter Mindestbetrag {min_amount_euro:.2f} EUR"
        ...
        if days_overdue < self.config.grace_days:
            return False, f"Rechnung {invoice.invoice_number} noch in Schonfrist ({self.config.grace_days} Tage)"
        ...
        if invoice.last_dunning_date:
            ...
            if days_since_last < 1:  # Minimum 1 day between dunning notices
                return False, f"Letzte Mahnung für {invoice.invoice_number} zu recent (vor {days_since_last} Tagen)"
        
        return True, None
    
    def calculate_dunning_fee(
        self,
        invoice: OverdueInvoice,
        stage: DunningStage
    ) -> int:
