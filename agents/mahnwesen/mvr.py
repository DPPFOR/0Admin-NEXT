...
class OverdueInvoice:
    ...
    last_dunning_sent: Optional[datetime] = None
...
class MVREngine:
    ...
    def should_send_dunning(
        ...
        last_dunning_date = getattr(invoice, 'last_dunning_sent', None) or getattr(invoice, 'last_dunning_date', None)
        if last_dunning_date:
            time_since_last = datetime.now(timezone.utc) - last_dunning_date
    ...
    def reset_rate_limits(self, tenant_id: Optional[str] = None):
