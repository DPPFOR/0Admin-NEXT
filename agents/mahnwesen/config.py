...
class DunningConfig:
    ...
    def from_tenant(cls, tenant_id: str) -> "DunningConfig":
        ...
        config.grace_days = int(os.getenv(f"{prefix}_GRACE_DAYS", config.grace_days))
        config.max_notices_per_hour = int(
            os.getenv(f"{prefix}_MAX_NOTICES_PER_HOUR", config.max_notices_per_hour)
        )
        config.default_locale = os.getenv(f"{prefix}_LOCALE", config.default_locale)
        config.read_api_base_url = os.getenv(f"{prefix}_READ_API_URL", config.read_api_base_url)
        config.company_name = os.getenv(f"{prefix}_COMPANY_NAME", config.company_name)
        config.company_address = os.getenv(f"{prefix}_COMPANY_ADDRESS", config.company_address)
        config.support_email = os.getenv(f"{prefix}_SUPPORT_EMAIL", config.support_email)
