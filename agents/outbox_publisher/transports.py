...
class StdoutTransport:
    ...
    def publish(
        self, tenant_id: str, event_type: str, payload_json: str, trace_id: str | None = None
    ) -> PublishResult:
...
class WebhookTransport:
    ...
    def __init__(self) -> None:
        ...
        self.timeout = httpx.Timeout(
            connect=settings.WEBHOOK_TIMEOUT_MS / 1000.0, read=settings.WEBHOOK_TIMEOUT_MS / 1000.0
        )
        self.success_codes = self._parse_success_codes(settings.WEBHOOK_SUCCESS_CODES)
        self.headers = self._sanitize_headers(
            self._parse_headers(settings.WEBHOOK_HEADERS_ALLOWLIST)
        )
        self.client = httpx.Client(timeout=self.timeout, verify=True, follow_redirects=False)
        self.domain_allow = [
            d.strip().lower()
            for d in (settings.WEBHOOK_DOMAIN_ALLOWLIST or "").split(",")
            if d.strip()
        ]
    ...
    def publish(
        ...
        try:
            resp = self.client.post(
                self.url, headers=self.headers, content=payload_json.encode("utf-8")
            )
