...
class FlockClient:
    ...
    def _make_request(
        ...
        self.logger.debug(
            "Flock request", extra={"request": request.to_dict(), "correlation_id": correlation_id}
        )
        ...
        try:
            response = self.session.request(
                method=method, url=url, headers=headers, data=request_data, timeout=timeout
            )
            ...
            self.logger.debug(
                "Flock response",
                extra={"response": flock_response.to_dict(), "correlation_id": correlation_id},
            )
